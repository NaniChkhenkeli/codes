...
def train_model_and_predict():
    ...
    df = df.sort_values(by=['Director', 'Year'])
    df['director_avg_revenue'] = (
        df.groupby('Director')['Gross Revenue (million)']
    ...
    y_pred_train = linear_svr_model.predict(X_train)
    y_pred_train = [max(0, pred) for pred in y_pred_train] 

    df_train = pd.DataFrame(X_train, columns=feature_columns)
    df_train['predicted_revenue'] = y_pred_train
    df_train['actual_revenue'] = y_train

    # decoding categorical features
    for col in categorical_columns:
        encoded_col = f'{col}_encoded'
        df_train[col] = label_encoders[col].inverse_transform(df_train[encoded_col].astype(int))

    # calculating predicted success
    df_train['predicted_success'] = (df_train['predicted_revenue'] >= mean_revenue).astype(int)
    df_train['actual_success'] = (df_train['actual_revenue'] >= mean_revenue).astype(int)
    ...
    df_output = df_train[output_columns].copy()
    ...
    print("\nTraining Statistics")
    print(f"Total movies in training set: {len(df_train)}")
    print(f"Predicted successes: {df_train['predicted_success'].sum()}")
    print(f"Actual successes: {df_train['actual_success'].sum()}")
    print(f"Correct predictions: {(df_train['predicted_success'] == df_train['actual_success']).sum()}")
    print(f"Training accuracy: {(df_train['predicted_success'] == df_train['actual_success']).mean():.2%}")
...
def analyze_and_visualize():
    ...
    precision = precision_score(df["actual_success"], df["predicted_success"])
    recall = recall_score(df["actual_success"], df["predicted_success"])
    f1 = f1_score(df["actual_success"], df["predicted_success"])
    ...
    # 5.Success rate by Genre (no filter)
...
def main():
    ...
    # Step 1: Train model and make predictions
    print("\n" + "=" * 60)
    print("STEP 1: TRAINING MODEL AND MAKING PREDICTIONS")
    print("=" * 60)
    mean_revenue = train_model_and_predict()
    
    # Step 2: Analyze results and create visualizations
    print("\n" + "=" * 60)
