...
def main():
    ...
    for combo in combos:
        ...
        if uses_title:
            ...
            clf.fit(X_train_combined, df.loc[train_idx,'Success'].values)
    ...
    if uses_title:
        ...
        X_train_full_combined = ensure_int_matrix(df.loc[train_idx, selected_non_title_cols].values)
        X_test_full_combined = ensure_int_matrix(df.loc[test_idx, selected_non_title_cols].values)
    ...
    for n in train_sizes:
        idxs = order[:n]
        clf = SVC(kernel='linear', random_state=42)
        clf.fit(X_train_full_combined[idxs,:], df.loc[train_idx,'Success'].values[idxs])
        y_pred_test = clf.predict(X_test_full_combined)
